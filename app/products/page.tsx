...
export default function ProductsPage() {
  ...
  const fetchVendors = async () => {
    ...
    try {
      ...
      const vendorsList = data.filter((user: User) => user.role && user.role !== 'superadmin').map((user: User) => ({...user, vendorLocation: user.location || 'N/A'}));
  ...
  const fetchProducts = async () => {
    ...
    setError('');
    try {
      const response = await fetch('https://ecom-rest-topaz.vercel.app/products');
      const data = await response.json();
  ...
  const handleAddProduct = async (e: React.FormEvent) => {
    ...
    try {
      const selectedVendor = vendors.find(v => String(v.id) === newProductForm.vendorId);
      const formData = {
  ...
  return (
    <div className="min-h-screen bg-white text-black">
      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
        <div className="mb-4">
          <h2 className="text-xl font-bold text-black mb-1">Products Management</h2>
          <p className="text-slate-500 text-sm">View all products in the catalog</p>
        </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    <th className="px-4 py-3 font-semibold text-gray-700 sticky left-0 bg-gray-50 z-10">ID</th>
                    ...
                    <tr key={product.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        {product.image || product.imagePath ? (
                          <img src={product.image || product.imagePath} alt={product.name} className="w-10 h-10 object-cover rounded border" />
                        ) : (
                      ...
                      <td className="px-4 py-3">
                        <span className={`px-2 py-1 rounded text-xs font-medium ${
                          (product.quantity || product.stock || 0) > 0 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                        }`}>
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex space-x-1 justify-end">
                          <button className="text-blue-600 hover:text-blue-800 text-xs p-1 hover:bg-blue-50 rounded transition">
                            ✏️ Edit
                          </button>
                          <button className="text-red-600 hover:text-red-800 text-xs p-1 hover:bg-red-50 rounded transition">
                            🗑️ Delete
                          </button>
                        </div>
                      </td>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4 max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={handleAddProduct} className="space-y-4">
                <div>
                  <label htmlFor="product-name" className="block text-sm font-medium text-black mb-1">
                    Name *
                  </label>
                  <input
                    id="product-name"
                ...
                <div>
                  <label htmlFor="product-details" className="block text-sm font-medium text-black mb-1">
                    Details
                  </label>
                  <textarea
                    id="product-details"
                ...
                <div>
                  <label htmlFor="product-price" className="block text-sm font-medium text-black mb-1">
                    Price (₹) *
                  </label>
                  <input
                    id="product-price"
                ...
                <div>
                  <label htmlFor="product-category" className="block text-sm font-medium text-black mb-1">
                    Category
                  </label>
                  <input
                    id="product-category"
                ...
                <div>
                  <label htmlFor="product-quantity" className="block text-sm font-medium text-black mb-1">
                    Quantity *
                  </label>
                  <input
                    id="product-quantity"
                ...
                <div>
                  <label htmlFor="product-image" className="block text-sm font-medium text-black mb-1">
                    Image URL
                  </label>
                  <input
                    id="product-image"
                    ...
                    onChange={(e) => setNewProductForm({...newProductForm, imagePath: e.target.value})}
                    className="w-full px-3 py-2 border border-gray-300 rounded focus:ring-1 focus:ring-slate-600 focus:border-slate-600 outline-none text-sm"
                  />
                </div>

                <div>
                  <label htmlFor="product-vendor" className="block text-sm font-medium text-black mb-1">
                    Vendor *
                  </label>
                  <select
                    id="product-vendor"
                    value={newProductForm.vendorId}
                    onChange={(e) => setNewProductForm({...newProductForm, vendorId: e.target.value})}
                  ...
                  {vendorsLoading && <p className="text-xs text-slate-500 mt-1">Loading vendors...</p>}
                </div>

                <div>
                  <label htmlFor="product-status" className="block text-sm font-medium text-black mb-1">
                    Status *
                  </label>
                  <select
                    id="product-status"
